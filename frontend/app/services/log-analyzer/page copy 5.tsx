...
export default function LogAnalyzerPage() {
 ...
 const handleDownloadPDF = () => {
  ...
  const renderReport = (data: any, filename: string) => (
    <div key={filename} className="border-t pt-6">
      ...
      {data.summary && (
        <section className="mb-6">
          ...
          <p className="text-gray-700">
            The log analysis detected <b>{data.summary.total_auth_failures}</b> authentication failures
            from <b>{data.summary.unique_ips}</b> unique IPs. There were
            <b> {data.summary.sessions_opened}</b> session openings and
            <b> {data.summary.sessions_closed}</b> closings. Additionally,
            <b> {data.summary.logrotate_alerts}</b> log rotation alerts were recorded.
          </p>
        </section>
      )}
      ...
      {data.observations && data.observations.length > 0 && (
        <section className="mb-6">
          ...
          <table className="w-full text-sm border-collapse border border-gray-300">
            ...
            <tbody>
              {data.observations.map((obs: any, i: number) => (
                <tr key={i}>
      ...
      {/* Traffic Patterns */}
      {data.traffic_patterns && data.traffic_patterns.high_auth_failure_ips && (
        <section className="mb-6">
          ...
          <ul className="list-disc ml-6 text-gray-700">
            {data.traffic_patterns.high_auth_failure_ips.map((ip: any, i: number) => (
              <li key={i}>
      ...
      {data.highest_severity && (
        <section className="mb-6">
          ...
          <p className="text-gray-700">
            The most critical IP is <b>{data.highest_severity.ip}</b> with{' '}
            <b>{data.highest_severity.failures}</b> failures, associated with user{' '}
            <b>{data.highest_severity.user}</b>.
          </p>
        </section>
      )}

      {/* Recommendations */}
      {data.planning?.recommendations?.length > 0 && (
        <section>
          <h3 className="text-lg font-semibold mb-2 text-emerald-700">Recommendations</h3>
          <ul className="list-disc ml-6 text-gray-700">
            {data.planning.recommendations.map((rec: string, i: number) => (
              <li key={i}>{rec}</li>
            ))}
          </ul>
        </section>
