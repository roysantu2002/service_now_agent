...
class LogAnalyzerService:
    ...
    - Generates PDF, Markdown, JSON reports (named after analysis_id)
    """
    ...
    async def analyze_log_file(
        ...
        try:
            ...
            logger.debug("AI RESPONSE TEXT", response=raw_text[:2000])  # print first 2K chars for safety
    ...
    async def ai_natural_query(self, log_data: str, question: str, context_size: int = 50) -> Dict[str, Any]:
        try:
            ...
            prompt, filtered_df = await self._prepare_ai_prompt(entries, question, context_size)
            ...
            logger.info("AI NATURAL QUERY RESPONSE RECEIVED", question=question)
            logger.debug("AI FULL RESPONSE TEXT", response=raw_text[:2000])
            ...
            tb = traceback.format_exc()
            logger.error("AI natural query failed", error=str(e), traceback=tb)
