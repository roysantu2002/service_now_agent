...
Knowledge graph building is disabled for chunk-only testing.
"""
...
try:
    ...
    from app.utils.db_utils import IngestionConfig, IngestionResult
except ImportError:
    import sys
...
class DocumentIngestionPipeline:
    """Pipeline for ingesting documents into vector DB (chunk-only mode)."""
    ...
    async def ingest_documents(
        ...
        """Ingest all markdown documents from folder."""
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Entities and graph disabled
        entities_extracted = 0
        relationships_created = 0
        graph_errors = []
        logger.info("Skipping entity extraction and knowledge graph building")
    ...
    async def _clean_databases(self):
        """Clean existing data from PostgreSQL only."""
        ...
        logger.info("Skipping graph cleaning (graph builder disabled)")


async def main():
    """Main function to run ingestion."""
    parser = argparse.ArgumentParser(description="Ingest documents into vector DB (chunk-only)")
    ...
    parser.add_argument("--fast", "-f", action="store_true", help="Fast mode: skip graph building")
    ...
    config = IngestionConfig(
        ...
        skip_graph_building=True  # Always skip graph
    )
    ...
    try:
        ...
        # Summary
        print("\n" + "="*50)
        print("INGESTION SUMMARY (Chunk-only)")
        print("="*50)
