...
def create_webhook_events_table() -> None:
    """Create webhook_events table for storing webhook data."""
    create_table_query = """
    ...
    CREATE INDEX IF NOT EXISTS idx_webhook_events_sys_id ON webhook_events(sys_id);
    CREATE INDEX IF NOT EXISTS idx_webhook_events_status ON webhook_events(status);
    CREATE INDEX IF NOT EXISTS idx_webhook_events_created_at ON webhook_events(created_at);
    """
    
    # This will be executed when tables are created
    logger.info("Webhook events table creation query prepared")


def create_incident_analysis_table() -> None:
    """Create incident_analysis table for storing AI analysis results."""
    create_table_query = """
    ...
    CREATE INDEX IF NOT EXISTS idx_incident_analysis_incident_id ON incident_analysis(incident_id);
    CREATE INDEX IF NOT EXISTS idx_incident_analysis_sys_id ON incident_analysis(sys_id);
    CREATE INDEX IF NOT EXISTS idx_incident_analysis_category ON incident_analysis(category);
    CREATE INDEX IF NOT EXISTS idx_incident_analysis_created_at ON incident_analysis(created_at);
    """
    
    # This will be executed when tables are created
    logger.info("Incident analysis table creation query prepared")
# -------------------------
...
async def upsert_webhook_event(
    ...
    """
    Insert or update a webhook event for a given incident_id.
    If the incident already exists, update its payload and metadata.
    """
    async with get_db_connection() as conn:
        row = await conn.fetchrow(
            """
...
async def initialize_tables() -> None:
    """Initialize database tables with indexes and upsert safety."""
    try:
        # --- Webhook Events ---
        await execute_query("""
            CREATE TABLE IF NOT EXISTS webhook_events (
        ...
        logger.info("✅ Database tables initialized and ready.")
    except Exception as e:
